...
def _choose_transfer_hints(table: ManifestTable) -> dict[str, Any]:
    ...
    if "(" in partition_key and ")" in partition_key:
        chunk_candidate = (
            partition_key.partition("(")[2].rpartition(")")[0].split(",")[0].strip().strip('"')
        )
    ...
    chunk_count = (
        1 if estimated_rows <= 250_000 else max(2, min(16, (estimated_rows // 250_000) + 1))
    )
...
def generate_plan(manifest_path: str, strategy: str = "largest_first") -> dict[str, Any]:
    ...
    mv_indexes = [
        item.model_dump(mode="python", by_alias=True) for item in manifest.matview_indexes
    ]
    ...
    partition_children = {
        (child.schema_name, child.table) for table in tables for child in table.partition.children
    }
    ...
    for schema in include_schemas:
        ...
        if schema_grants.get(schema):
            step_i = _append_step(
                steps, step_i, op="apply_grants", schema=schema, grants=schema_grants[schema]
            )
    ...
    for table_info in root_tables:
        ...
        if (
            verify_small_tables
            and isinstance(table_info.estimated_rows, int)
            and table_info.estimated_rows <= small_table_rows
        ):
        ...
        if partition.is_partition_parent and partition.children:
            for child in partition.children:
                step_i = _append_step(
                    steps, step_i, op="ensure_table", schema=child.schema_name, table=child.table
                )
                step_i = _append_step(
                    steps,
                    step_i,
        ...
        if indexes:
            step_i = _append_step(
                steps, step_i, op="create_indexes", schema=schema, table=table, indexes=indexes
            )
        ...
        step_i = _append_step(
            steps, step_i, op="verify_table", schema=schema, table=table, validate=validate
        )

        maintenance_op = (
            "vacuum_analyze_table"
            if isinstance(table_info.estimated_rows, int) and table_info.estimated_rows >= 100_000
            else "analyze_table"
        )
        step_i = _append_step(
            steps,
            ...
            table=table,
            maintenance={
                "cluster_if_needed": bool(
                    indexes and any(index.get("cluster_statement") for index in indexes)
                )
            },
        )
    ...
    for schema in include_schemas:
        ...
        if table_grants:
            step_i = _append_step(
                steps, step_i, op="apply_grants", schema=schema, grants=table_grants
            )
    ...
    for schema in include_schemas:
        ...
        if staged_mvs:
            step_i = _append_step(
                steps, step_i, op="stage_matviews", schema=schema, matviews=staged_mvs
            )
        if schema_mvs:
            step_i = _append_step(
                steps, step_i, op="create_matviews", schema=schema, matviews=schema_mvs
            )

        schema_mv_indexes = [item for item in mv_indexes if item["schema"] == schema]
        if schema_mv_indexes:
            step_i = _append_step(
                steps, step_i, op="create_mv_indexes", schema=schema, indexes=schema_mv_indexes
            )
        matview_grants = [grant for mv in schema_mvs for grant in mv.get("grants", [])]
        if matview_grants:
            step_i = _append_step(
                steps, step_i, op="apply_grants", schema=schema, grants=matview_grants
            )
