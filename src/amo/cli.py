...
from amo.core.agent import MigrationAgent
...
def approve(
    ...
    summary: str = typer.Option(..., help="Path to pre_migration_summary.json"),
    source_manifest: str = typer.Option(..., help="Path to source_manifest.json"),
...
@app.command("dry-run")
def dry_run(
    plan: str | None = typer.Option(None, help="Optional relocated path to the approved plan"),
    ...
    try:
        bundle = build_approved_execution_bundle(approval_path=approval, plan_path=plan)
...
def run(
    ...
    execute(cfg=cfg, bundle=bundle, state_path=state)
...
def retry(
    ...
    if source_state.get("schema_version") != "2" or not source_state.get("plan_sha256"):
        raise typer.BadParameter(
            "Source state predates plan integrity binding. Retry requires a schema-v2 state file."
        )
    if source_state["plan_sha256"] != bundle.plan_sha256:
        raise typer.BadParameter("Source state belongs to a different approved plan.")
    retry_plan, retry_summary = build_retry_plan(
        plan=bundle.filtered_plan,
        state=source_state,
        mode=mode,
        table=table,
    )
    ...
    execute(cfg=cfg, bundle=bundle.with_filtered_plan(retry_plan), state_path=retry_state)
...
def agent_run(
    ...
    out_dir: str | None = typer.Option(None, help="Output directory for agent artifacts"),
    ...
    execute: bool = typer.Option(
        False,
        "--execute",
        help="Execute after dry-run validation. Requires --approval.",
    ),
    state: str | None = typer.Option(None, help="State path for execution"),
):
    ...
    results = [agent.observe(), agent.plan(), agent.critique()]

    if approval:
        dry_run_result = agent.dry_run(approval_path=approval)
        results.append(dry_run_result)
        if execute:
            if not dry_run_result.ok:
                typer.echo(f"Agent run blocked: {dry_run_result.message}")
                typer.echo(f"Agent trace: {agent.trace_path}")
                raise typer.Exit(code=1)
            execute_result = agent.execute_approved(approval_path=approval, state_path=state)
            results.append(execute_result)
            if execute_result.ok:
                results.append(agent.verify())
                results.append(agent.summarize(state_path=state))
    else:
        blocked = agent.dry_run()
        results.append(blocked)
        if execute:
            typer.echo("Agent run blocked: --execute requires --approval.")
            typer.echo(f"Agent trace: {agent.trace_path}")
            raise typer.Exit(code=1)

    for result in results:
        status = "OK" if result.ok else "BLOCKED"
        typer.echo(f"{status} {result.phase.value}: {result.message}")
